"""COMP1510 Assignment # 3: Your First SUD!"""

# Trae Bold
# A01072453
# JAN 11, 2019


def print_candy_reward():
    print("""         o   oo                     
     o         o   o                
        ___o o   o                  
       /`._;o       o               
      |   /   o                     
      ;_  |                        
        `-'                       
   Bags of candy$$$          \n\n""")


def print_exit():
    print("""
                                                888888b
                                                8888888
                                                8888888
                                                8888888
                                     _          8888888
                                   ,d88         8888888
                            ____  d88' _,,      888888'
                           (8888\ 88' d888)     Y8888P
                           ___~~8 ~8  88~___    d8888
  _______              ,8888888        ~ 888888_8888
,8888888888===__    _,d88P~~               ~~Y88888'
88888888888888888888888'                        `88b
8888888888888888888888P                          Y88
`~888888888888~~~~~ 88                            88
    ~~~~~~~~        88                            88
                    88                            88
                    88                            88
                    88                            88
                    88    ,aa.            ,aa.    88
                    88    d88b            d88b    88
                  ,=88    Y88P            Y88P    88=,
                ,d88P'     `'     _aa_     `'     `Y88b,     ___
                88P'             (8888)             `Y88  ad88888b
                88                ~^^~                88 d88Y~~"Y8b
         _______"Yb._                              _.d8"d8Y      88
 ______,d88888888ba888=,.______________________.,=8888~d88_______88___
|~~~~~~88P~~~~~~Y88~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|
|      88        88                                                   |
|      88        88    hi,                                            |
|      88ba,___,d8P                                                   |
|       "888888888       Thank you for playing my game!               |
|          ~~~~~~                                                     |
|       Dear Warrior Kitty, your bravery will be acknowledged         | 
|   and I personally would like to thank you for standing up against  |
|   those hideous monsters. The Kingdom of Hershey will always be safe|
|   in your hands.                                                    |
|        It is my first SUD game and I am very grateful that you got  |
|   to play my first ever game!                                       |
|        Please come back again!                                      |
|                                               |\      _,,,--,,_  ,) |
|    Sincrely, Trae Bold                        /,`.-'`'   -,  ;-;;'  |
|_____________________________________________ |,4-  ) )-,_ ) /\______|
 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'---''(_/--' (_/-'~~~~~~\n\n""")


def print_intro():
    print("""    
           / \                                                       / |
          |  |                                                      |  |
         /----\                  Kingdom of Hershey                /----|
        [______]             Where Happy Kitties Reside          [______]
         |    |         _____                        _____         |    |
         |[]  |        [     ]                      [     ]        |  []|
         |    |       [_______][ ][ ][ ][][ ][ ][ ][_______]       |    |
         |    [ ][ ][ ]|     |  ,----------------,  |     |[ ][ ][ ]    |
         |             |     |/'    ____..____    '\|     |             |
          \  []        |     |    /'    ||    '\    |     |        []  /
           |      []   |     |   |o     ||     o|   |     |  []       |
           |           |  _  |   |     _||_     |   |  _  |           |
           |   []      | (_) |   |    (_||_)    |   | (_) |       []  |
           |           |     |   |     (||)     |   |     |           |
           |           |     |   |      ||      |   |     |           |
         /''           |     |   |o     ||     o|   |     |           ''|
        [_____________[_______]--'------''------'--[_______]_____________]\n\n""")


def print_intro_1():
    print(
        """Once upon a time there was a kingdom called Hershey  🍭 🍭 🍭

    Fields of licorice, trees with gleaming shining green-apple Jolly Ranchers 🍏

    It is all candy, everything is made out of candy 🍬🍬🍬

    With a joy in their eyes, there resides the kittens, citizens of Hershey Kingdom

    But havoc spread in the kingdom, when the insatiable monsters of Toblerone 👹👹👹started

    attacking and stealing candies from the sweet, delicious and sugary Hershey Land...

    Only hope to restore balance is to find a brave warrior kitty to face the enemies...⚔⚔⚔

    JOIN US! fight the enemy and you shall receive the spoil of eternal candies🎁🎁🎁...\n\n """)


def print_happy_kitty():
    print(""" _   _         _  _          _   __ _
    | | | |  ___  | || |  ___   | | / /(_)  _     _
    | |_| | / _ \ | || | / _ \  | |/ /  _ _| |_ _| |_  _  _
    |  _  |/ /_\ \| || |/ / \ \ |   /  | |_   _|_   _|| |/ /
    | | | |\ ,___/| || |\ \_/ / | |\ \ | | | |_  | |_ | / /
    |_| |_| \___/ |_||_| \___/  |_| \_\|_| \___| \___||  /
                           _           _              / /
                          / \_______ /|_\             \/
                         /          /_/ \__
                        /             \_/ /
                      _|_              |/|_
                      _|_  O    _    O  _|_
                      _|_      (_)      _|_
                       \                 /
                        _\_____________/_
                       |  \/  (___)  \/  |
                       \__(  o     o  )__/
                       """'\n\n')


def print_death():
    print("""/\
||_____-----_____-----_____
||   O                  O  \
||    O\\    ___    //O    /
||       \\ /   \//        \
||         |_O O_|         /
||          ^ | ^          \
||        // UUU \\        /
||    O//            \\O   \
||   O                  O  /
||_____-----_____-----_____\
||
||.\n\n""")